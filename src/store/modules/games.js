    ...
    return action$.pipe(
        ...
        mergeMap(([action, state]) => {
            ...
            if(localStorage.getItem("userInfo"))
                header = Object.assign(header, { Authorization: `token ${JSON.parse(localStorage.getItem("userInfo")).token}` })
        return ajax
            .get(`/games/?${queryString.stringify(query)}`, header)
            .pipe(
                map(response => {
...
const createRatingEpic = (action$, state$) => {
    return action$.pipe(
        ...
        return ajax
            .post(
            `/ratings/create/?type=game&slug=${slug}`,
...
const createReviewEpic = (action$, state$) => {
    return action$.pipe(
        ...
        return ajax
            .post(
            `/reviews/create/?type=game&slug=${slug}`,
